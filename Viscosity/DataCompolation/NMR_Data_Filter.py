...
def is_valid_molecule(inchi):
    ...
    if num_carbons < 3:
        return False
    #remove atoms with more than 2 aromatic bonds
    for atom in mol.GetAtoms():
        if atom.GetIsAromatic() and len([bond for bond in atom.GetBonds() if bond.GetIsAromatic()]) > 2:
            return False
    #remove if more than one benzene ring
    if len([1 for atom in mol.GetAtoms() if atom.GetIsAromatic()]) > 6:
        return False
...
def get_functional_groups(mol):
    ...
    patterns = {
        ...
        'Benzyl_Rings': Chem.MolFromSmarts('c1ccccc1'),
