...
for i in range(len(db_1H)):
    ...
    if db_1H[i]['Intensity'] is None or db_1H[i]['Frequency (ppm)'] is None:
...
for i in range(len(peaks_x)):
    plt.scatter(peaks_x[i], peaks_y[i]/np.max(peaks_y[i]), c=[color_marker[labels[i] % len(color_marker)][0]], marker=color_marker[labels[i] % len(color_marker)][1])
# Plot real spectra with low alpha
for i in range(len(db_1H)):
    plt.plot(db_1H[i]['Frequency (ppm)'], db_1H[i]['Intensity']/np.max(db_1H[i]['Intensity']), label=db_1H[i]['Smiles'], alpha=0.1)
plt.xlabel('Ppm')
...
# Reconstruct Y from the model and calculate MAPE
y_pred_A = model_A.predict(features)
y_pred_B = model_B.predict(features)

y_pred = y_pred_A + y_pred_B*100

mape = mean_absolute_percentage_error(target_A + target_B*100, y_pred)
print(f"Mean Absolute Percentage Error for A and B combined: {mape:.2f}%")
